...
import carnepicada from "@/Assets/Images/cif-limpiavidrios-recarga.jpg";
...
const Card = ({
  ...
  textName: string;
  elementsArray: itemsTest[];
}) => {
  let elementsText = null;

  if (elementsArray.length <= 3) {
    elementsText = elementsArray.map((element: itemsTest) => (
      <Text
        key={element.id}
        className=" text-red-600 text-center text-xl rounded-full"
      >
        {element.name}
      </Text>
    ));
    ...
    elementsText = newElementsArray.map((element: itemsTest) => (
      <Text
        key={element.id}
        className="text-red-600 text-center text-xl rounded-full"
      >
  ...
  return (
    <View className="flex items-center justify-center h-44 w-44 border border-y-blue-700 rounded-full  overflow-hidden">
      <Image
        source={carnepicada}
        className="absolute  h-44 w-44 z-0 opacity-60"
      />
